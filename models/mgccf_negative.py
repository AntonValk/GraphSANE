...
def get_unique_index(input, unique_vals):
    res = tf.map_fn(
        lambda x: tf.argmax(tf.cast(tf.equal(input, tf.cast(x, tf.int32)), tf.int64)),
        unique_vals, dtype=tf.int64)
    return res
...
def calculate_contrastive_loss2(old_embedding, old_num, cur_pos_neigh, cur_neg_neigh, tau, selected_id,
                                user_weight=None):
    '''
...
class WeightGenerator(object):
    ...
    def __call__(self, state_vector):
        ...
        for dense_layer in self.mlp_layers:
            state_vector, model_weight = dense_layer(state_vector)
            model_weights.append(model_weight)
        user_weights = tf.matmul(state_vector, self.weight)
        user_weights = tf.math.softplus(user_weights)
        model_weights.append(self.weight)
        return user_weights, model_weights
...
class GNN(object):
    def __init__(self, dataset_argv, architect_argv, adj_degree, num_samples,
                 ptmzr_argv, aggregator, act, neigh_drop_rate, l2_embed, dist_embed, wg_embed=0, num_self_neigh=10,
                 neg_item_num=10, ui_con_positive=15, con_ratios=[1, 1, 1, 1, 1, 1, 1, 1], trans_positive=10,
                 pretrain_data=None, old_i_centers = None,
                 inc_reg=[0, 0, 0], old_num_user=0, old_num_item=0, distill_mode='', k_centroids=[0, 0], tau=0,
                 num_neigh=15,
                 local_distill_mode='', contrastive_mode='', layer_wise=None, layer_l2_mode=None,
                 lambda_layer_l2=[0, 0, 0], lambda_contrastive=[0, 0, 0], lambda_soft=0,
                 adaptive_mode='', wg_dimension=128, center_initialize=None, soft_center=None, wg_act='tf.nn.relu',
                 nu=1, hard_pos=False):
        '''
        ...
        :param neg_item_num: number of negative paris for each positive pair in bpr loss
        ...
        with self.graph.as_default():
            ...
            self.hard_pos_item = tf.placeholder(tf.int32, shape=[None, 2], name='hard_pos_item')
            ...
            self.old_user_medium_input_1 = tf.placeholder(tf.float64, shape=[None, self.layer_dims[1]],
                                                          name='u_old_medium_1')
            self.old_item_medium_input_1 = tf.placeholder(tf.float64, shape=[None, self.layer_dims[1]],
                                                          name='i_old_medium_1')
            self.old_user_medium_input_2 = tf.placeholder(tf.float64, shape=[None, self.layer_dims[2]*3],
                                                          name='u_old_medium_2')
            self.old_item_medium_input_2 = tf.placeholder(tf.float64, shape=[None, self.layer_dims[2]*3],
                                                          name='i_old_medium_2')
            # For layer_wise
            ...
            # self.old_i_soft_center = tf.placeholder(tf.float64, shape=[self.k_centroids[1], self.layer_dims[2]],
            #                                         name="old_i_soft_center")
            ...
            if not old_i_centers is None:
                self.old_i_soft_center = old_i_centers
                self.all_q = self.get_Q(self.old_item_medium_input_2[:, :self.layer_dims[2]], old_i_centers)
                self.inc_flag = True
            else:
                self.all_q = tf.random.uniform([self.num_item, self.k_centroids[1]])
            ...
            self.softkl_loss, self.interest_shift = self.model_fn('model')
            assert ptmzr_argv[0].lower() == 'adam'
            _learning_rate, _epsilon = ptmzr_argv[1:3]
            update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
            with tf.control_dependencies(update_ops):
                assert ptmzr_argv[0].lower() == 'adam'
                self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=_learning_rate,
                                                              epsilon=_epsilon)
                ...
                self.grads_and_vars = self.optimizer.compute_gradients(self.bpr_loss + self.reg_loss + self.dist_loss)
                self.gradients = [tf.reduce_sum(grad) for grad,var in self.grads_and_vars if grad is not None]
                grad_index = [idx for idx, (grad,var) in enumerate(self.grads_and_vars) if grad is not None]
                variable_names = [v.name for v in tf.trainable_variables()]
                self.variable_names = [variable_names[i] for i in grad_index]
                self.ptmzr = self.optimizer.apply_gradients(self.grads_and_vars)

            self.grads = tf.gradients(self.bpr_loss + self.reg_loss + self.dist_loss, tf.trainable_variables())
    def model_fn(self, scope):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            self.user_medium_input_1 = tf.Variable(initializer([self.num_item, self.layer_dims[1]]), trainable=False,
                                                   name='user_medium_input_1')
            self.item_medium_input_1 = tf.Variable(initializer([self.num_user, self.layer_dims[1]]), trainable=False,
                                                   name='item_medium_input_1')
            self.user_medium_input_2 = tf.Variable(initializer([self.num_user, self.layer_dims[2]*3]), trainable=False,
                                                   name='user_medium_input_2')
            self.item_medium_input_2 = tf.Variable(initializer([self.num_item, self.layer_dims[2]*3]), trainable=False,
                                                   name='item_medium_input_2')
            if not self.center_initialize:
                self.i_soft_centers = tf.Variable(initializer([self.k_centroids[1], self.layer_dims[2]]), trainable=True,
                                                  name="i_center", dtype=tf.float64)
            else:
                self.i_soft_centers = tf.Variable(initial_value=self.old_i_soft_center, trainable=True,
            ...
            unique_u_id, _ = tf.unique(self.u_id)
            unique_u_idx = get_unique_index(self.u_id, unique_u_id)
            selected_u_idx = tf.where(unique_u_id < self.old_num_user)[:, -1]
            selected_u_id = tf.nn.embedding_lookup(tf.cast(unique_u_id, dtype=tf.int64), selected_u_idx)
            # i_id = tf.concat([self.pos_item_id, tf.reshape(self.neg_item_id, [-1, ])], axis=0)
            unique_i_id, _ = tf.unique(self.pos_item_id)
            unique_i_idx = get_unique_index(self.pos_item_id, unique_i_id)
            selected_i_idx = tf.where(unique_i_id < self.old_num_item)[:, -1]
            selected_i_id = tf.nn.embedding_lookup(tf.cast(unique_i_id, dtype=tf.int64), selected_i_idx)
            ...
            all_user_rep, all_user_embed, user_medium_input_1, user_medium_id_1 = self.graphconv(
                'user_gcn', tf.concat([self.u_id, pos_users], 0),
            ...
            self.user_medium_input_1 = tf.tensor_scatter_nd_update(self.user_medium_input_1, tf.expand_dims(user_medium_id_1, 1), user_medium_input_1)
            unique_u_rep = tf.nn.embedding_lookup(user_rep, unique_u_idx)
            self.user_medium_input_2 = tf.tensor_scatter_nd_update(self.user_medium_input_2, tf.expand_dims(unique_u_id, 1), unique_u_rep)

            # ================================================================
            pos_items_neighs = tf.reshape(uniform_sample(self.pos_item_id, self.v_v_graph_ph, 1), [-1, ])
            all_pos_item_rep, all_pos_item_embed, pos_item_medium_input_1, pos_item_medium_id_1 = self.graphconv(
                'pos_item_gcn',
            ...
            self.item_medium_input_1 = tf.tensor_scatter_nd_update(self.item_medium_input_1, tf.expand_dims(pos_item_medium_id_1, 1), pos_item_medium_input_1)
            unique_i_rep = tf.nn.embedding_lookup(pos_item_rep, unique_i_idx)
            self.item_medium_input_2 = tf.tensor_scatter_nd_update(self.item_medium_input_2, tf.expand_dims(unique_i_id, 1), unique_i_rep)
            # ============================
            neg_item_rep, neg_item_embed = self.graphconv(
                'neg_item_gcn', self.neg_item_id, self.user_embeddings,
                self.item_embeddings, 'neg_item', dimen_1=self.neg_item_num)
            ...
            hard_pos_item_rep, hard_pos_item_embed = self.graphconv(
                'pos_item_gcn', self.hard_pos_item, self.user_embeddings,
                self.item_embeddings, 'neg_item', dimen_1=2)
            hard_pos_item_rep = tf.concat([hard_pos_item_rep, hard_pos_item_embed], 2)

            if self.contrastive_mode or self.adaptive_mode:
                medium_u_idx = tf.where(pos_item_medium_id_1 < self.old_num_user)[:, -1]
                medium_u_id = tf.nn.embedding_lookup(tf.cast(pos_item_medium_id_1, dtype=tf.int64), medium_u_idx)
                medium_i_idx = tf.where(user_medium_id_1 < self.old_num_item)[:, -1]
                medium_i_id = tf.nn.embedding_lookup(tf.cast(user_medium_id_1, dtype=tf.int64), medium_i_idx)

            # === BPR loss
            if self.hard_pos:
                batch_hard_pos_item_embedding = tf.transpose(hard_pos_item_rep, [0, 2, 1])
                hard_pos_rating = tf.matmul(tf.expand_dims(user_rep, 1), batch_hard_pos_item_embedding)
                hard_pos_rating = tf.squeeze(hard_pos_rating, 1)
                hard_pos_rating = tf.reshape(hard_pos_rating, [tf.shape(hard_pos_rating)[0], 2])

            pos_rating = tf.reduce_sum(tf.multiply(user_rep, pos_item_rep), 1)
            if self.hard_pos:
                pos_rating = tf.expand_dims(pos_rating, 1)
                pos_rating = tf.reduce_mean(tf.concat([hard_pos_rating, pos_rating], axis=1), axis=1)
            ...
            q = self.get_Q(unique_i_rep[:, :self.layer_dims[2]], self.i_soft_centers)
            p = self.get_P(q)

            if self.inc_flag:
                interest_shift = self.get_interest_shift(self.old_i_soft_center, selected_u_id)
            else:
                interest_shift = tf.random.uniform([self.num_user, self.k_centroids[1]])
            ...
            if self.adaptive_mode == "SW-AIW-soft-base":
                q = self.get_Q(unique_i_rep[:, :self.layer_dims[2]], self.i_soft_centers)
                p = self.get_P(q)
                softcluster_kl = tf.reduce_sum(tf.reduce_sum(
                    tf.multiply(p, tf.log(tf.divide(p, q))), axis=-1))
            ...
            if self.inc_reg[1] > 0:
                if self.local_distill_mode == 'LSP_s':
                    ...
                    if not self.adaptive_mode:
                        ...
                        loss_ui = tf.reduce_mean(
                            user_weight * tf.nn.embedding_lookup(tf.squared_difference(u_i_prod_new, self.ui_dist),
                                                                 selected_u_id))
            ...
            if self.global_distillation_flag:
                ...
                if not self.adaptive_mode:
                    ...
                    u_ua_kl = tf.reduce_mean(
                        user_weight * tf.nn.embedding_lookup(tf.reduce_sum(tf.multiply(u_u_a_gs_matrix, tf.log(
                            tf.divide(u_u_a_gs_matrix, self.old_user_gs[:, :self.k_centroids[0]]))), 1), selected_u_id))
                    u_ia_kl = tf.reduce_mean(
                        user_weight * tf.nn.embedding_lookup(tf.reduce_sum(tf.multiply(u_i_a_gs_matrix, tf.log(
            ...
            if self.contrastive_mode or self.adaptive_mode or self.layer_wise:
                selected_u_embed = tf.nn.embedding_lookup(tf.nn.embedding_lookup(user_embed, unique_u_idx), selected_u_idx)
                selected_i_embed = tf.nn.embedding_lookup(tf.nn.embedding_lookup(pos_item_embed, unique_i_idx), selected_i_idx)
                selected_u_medium = tf.nn.embedding_lookup(pos_item_medium_input_1, medium_u_idx)
                selected_i_medium = tf.nn.embedding_lookup(user_medium_input_1, medium_i_idx)
                selected_u_rep = tf.nn.embedding_lookup(unique_u_rep, selected_u_idx)
                selected_i_rep = tf.nn.embedding_lookup(unique_i_rep, selected_i_idx)
                user_inputs = [selected_u_embed, selected_u_medium, selected_u_rep]
                item_inputs = [selected_i_embed, selected_i_medium, selected_i_rep]
                #
                # old_selected_u_embed = tf.nn.embedding_lookup(self.old_user_embedding, selected_u_id)
                # old_selected_i_embed = tf.nn.embedding_lookup(self.old_item_embedding, selected_i_id)
                # old_selected_u_medium = tf.nn.embedding_lookup(self.old_item_medium_input_1, medium_u_id)
                # old_selected_i_medium = tf.nn.embedding_lookup(self.old_user_medium_input_1, medium_i_id)
                # old_selected_u_rep = tf.nn.embedding_lookup(self.old_user_medium_input_2, selected_u_id)
                # old_selected_i_rep = tf.nn.embedding_lookup(self.old_item_medium_input_2, selected_i_id)
                ...
                user_indexes = [selected_u_id, medium_u_id, selected_u_id]
                item_indexes = [selected_i_id, medium_i_id, selected_i_id]
            ...
            if self.contrastive_mode:
                if not self.layer_wise:
                    ct_loss = self.calc_contrastive(old_user_inputs[0], old_item_inputs[0],
                                                    user_inputs[0], item_inputs[0],
                                                    None, user_indexes[0], item_indexes[0])
                    contrastive_loss.append(self.lambda_contrastive[0] * ct_loss)
                else:
                    for i in range(self.num_layers):
                        ct_loss = self.calc_contrastive(old_user_inputs[i], old_item_inputs[i],
                                                        user_inputs[i], item_inputs[i],
                                                        None, user_indexes[i], item_indexes[i])
                        contrastive_loss.append(self.lambda_contrastive[i] * ct_loss)
                contrastive_loss = tf.reduce_sum(tf.stack(contrastive_loss))
        ...
        return bpr_loss, reg_loss, self.dist_embed * (user_user_distance + pos_item_item_dist), \
               contrastive_loss, softkl_loss, interest_shift
    ...
    def get_interest_shift(self, old_i_centers, selected_u_id):
        trans_old_i_anchors = old_i_centers
        old_selected_u_embed = tf.nn.embedding_lookup(self.old_user_embedding, selected_u_id)
        selected_u_embed = tf.nn.embedding_lookup(self.user_embeddings, selected_u_id)
        old_click_mat = tf.matmul(trans_old_i_anchors,
                                  tf.transpose(old_selected_u_embed))
        click_mat = tf.matmul(trans_old_i_anchors,
                              tf.transpose(selected_u_embed))

        old_click_mat = tf.transpose(tf.stack(old_click_mat, axis=0))
        click_mat = tf.transpose(tf.stack(click_mat, axis=0))

        old_click_mat = old_click_mat/tf.expand_dims(tf.reduce_sum(old_click_mat, axis=-1), axis=1)
        click_mat = click_mat/tf.expand_dims(tf.reduce_sum(click_mat, axis=-1), axis=1)
        interest_diff = tf.nn.softmax(old_click_mat-click_mat / self.tau, axis=-1)
        return interest_diff

    def get_state_vector(self, old_i_centers, selected_u_id, cluster_trans_mat):
        if cluster_trans_mat != None:
    ...
    def graphconv(self, scope, central_ids, user_embeddings, item_embeddings, tag, medium=False, dimen_1=None):
        with tf.compat.v1.variable_scope(scope):
            if tag == 'user':
            ...
            for i in range(self.num_layers - 2, -1, -1):
                ...
                if medium:
                    ...
                    medium_idx = get_unique_index(self_id_at_layers[i], medium_id)
                    medium_input = tf.nn.embedding_lookup(output1, medium_idx)
                    if i == self.num_layers - 2:
                        medium_id_1 = medium_id
                        medium_input_1 = medium_input

                    # elif i == self.num_layers - 3:
                    #     medium_id_2 = medium_id
                    #     medium_input_2 = medium_input
                ...
                if tag != "pos_item" and tag != "user":
                    ...
                    output = tf.reshape(output, [tf.shape(embed)[0], dimen_1, output_shape[-1]])
            if not medium:
                return output, embed
            else:
                return output, embed, medium_input_1, medium_id_1
