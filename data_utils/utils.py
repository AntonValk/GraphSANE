...
def generate_sparse_adj_matrix(train_set, num_users, num_items, user_shift=0, item_shift=0, skip_list=[]):
    '''
    ...
    :param skip_list: users not to sample
...
def generate_sparse_adj_matrix_pagerank(train_set, num_users, num_items, skip_list=[]):
    '''
    Generate user_item sparse adjacent matrix for all nodes
    :param train_set: list of lists of users' test item neighbors (length of number of users)
    :param num_users: number of users
    :param num_items: number of items
    :param user_shift: user index shift number
    ...
    col = np.array(col)
    data = np.array(data)
    rating_matrix = sp.csr_matrix((data, (row, col)), shape=(num_users+num_items, num_items+num_users))
    return rating_matrix
